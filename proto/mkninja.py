...
# set up a dynamic dependency on all proto files
protofiles = tools.manifest(
    where=SRC,
    cmd=["find", "src/determined", "-type", "f", "-name", "*.proto"],
    out=BLD/"protofiles"
)

grpc_in = "src/determined/api/v1/api.proto"
swagger_in = "src/determined/api/v1/api.proto"
build_path = "build/proto/github.com/determined-ai/determined/proto/pkg"
stamp = BLD/"proto.stamp"

protobuild = add_target(
    inputs=[protofiles, SRC/"build_proto.sh"],
    outputs=[stamp],
    command=["./build_proto.sh", BLD],
    workdir=SRC,
    display="generating proto files",
)
